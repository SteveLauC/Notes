...
3. shell sort
   ...
   `j_idx -= 1` will elicit overflow. We alreay the step of previous impl is `0`,
   ...
   even though we take the value of `j_idx` to its smallest value `start`, if `start > step`,
   ...
   The basic idea of shell sort:

   If we sort a array like `[9, 8, 7, 6, 5, 4, 3, 2, 1]` using plain `insertion sort`,
   note the last item `1` needs to be moved to the start of the array using 8 steps,
   ...
   decreases to 2. And this is the enhancement of our `shell sort`.
