...
10. 候选函数(candidate function)与可行函数(viable function)
    
    函数进行重载的时候，可选的可调用函数集合就叫做`候选函数`，`候选函数`具备2个
    特征，一是与被调用的函数同名，二是其声明在调用点可用(候选函数不在乎参数)
    ...
    比如最上面的`f`函数就有上面4个候选函数

    可行函数是在候选函数中选出参数匹配上的，其也有2个特征：一是形参数量和提供的
    实参的数量相同，二是实参的类型和对应的形参的类型相同或者可以转化为形参的类型
    ...
    在确定完可行函数之后，编译器就会在可行函数中选择最优的匹配，其基本思想是实参
    和形参的类型越接近，他们越匹配。基于上述信心，`f`的最优匹配是`void f(double, double = 3.14)`
    因为`f(5.6)`和`double`更加接近
