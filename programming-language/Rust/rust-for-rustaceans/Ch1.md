...
23. generic lifetime
    当定义的`struct`包含引用时，需要给引用加上生命周期标记，这样是为了在其实现的方
    法中判断利用生命周期参数来判断引用的合法性，压制悬垂指针。

24. 当你定义的类型中有引用时，需要有2点值得注意:
    1. 假如你的类型实现了`Drop`，那么在调用`drop`前，`borrow checker`仍会检查类型中
    引用的合法性(i.e.死之前也要检查下，检查好了才让你死)
...
25. 在思考结构体的生命周期范型参数时，对于结构体判断两个方面，1是其实例的构造，想
    ...
    这两个场景要连在一起看，比如`24`点中的代码，1/2是在一起的。
