...
3. Database files
   ...
   index also has a free space map, stores information about available space
   in the main file, it is named `filenode_fsm`.
...
4. Some tables' `relfilenode`s are 0, 0 is just an indicator indicating that the
   real `relfilenode` is stored in a low level data strucutre, and can be obtained 
...
5. By default, when a table file (fork)'s exceeds 1 GiB, it will be spliited to 
   ...
   this probably won't happen in practise.
