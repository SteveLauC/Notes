...
* targetList: (List<TargetEntry>)
  ...
  * For `INSEET`: the row that will be inserted into the table. During parser and 
  ...
  * For `UPDATE`: In analyzer, it represent the columns that will be updated. 
    ...
    Rewriter will merge the entires with the same `resno`
    
    Planner extracts resno
